const API_URL = 'https://lavender-dorian-singer.glitch.me/';
...
const renderPrice = (wrapper, data) => { // wrapper = priceList; data = data
    data.forEach((item) => {
...
const initService = () => {
    ...
    const bookFieldsetService = document.querySelector('.book__fieldset_service');
    priceList.textContent = '';
    addPreload(priceList);

    bookFieldsetService.innerHTML = '<legend class="book__legend">Услуга</legend>';
    addPreload(bookFieldsetService);
    ...
    fetch(`${API_URL}/api`)
        ...
        .then(data => {
            renderService(bookFieldsetService, data);
            removePreload(bookFieldsetService);
        })
}
...
const initBook = () => {
    ...
    bookForm.addEventListener('change', async (event) => { // change - смотрит не прошло ли изменение внутри внутри формы, если прошло, то запускает скрипт. 
        ...
        if (target.name === 'service') {
            addDisabled([fieldspec, fielddata, fieldmonth, fieldday, fieldtime, btn]);
            fieldspec.innerHTML = '<legend class="book__legend">Специалист</legend>';
        ...
        if (target.name === 'month') {
            ...
            addPreload(fieldday);
            const response = await fetch(`${API_URL}/api?spec=${bookForm.spec.value}&month=${bookForm.month.value}`);
            const data = await response.json()
            fieldday.textContent = ''; 
            renderDay(fieldday, data, bookForm.month.value); 
        ...
        if (target.name === 'day') {
            ...
            const response = await fetch(`${API_URL}/api?spec=${bookForm.spec.value}&month=${bookForm.month.value}&day=${target.value}`);
            const data = await response.json()
